class mpi_parallel_softmax_error
{
public:
    float avg=0;
    /*
...
#pragma omp parallel for private(j)
    ...
    uint32 percentage_of_mach_found(float **out,int root_id,uint16 x_cl,uint16 y_cl)
    {
        float *t=new float[y_cl];
        uint32 sum=0;uint16 i,j;
        MPI_Status st;
        #pragma omp parallel for private(j,t,st)  reduction( + : sum )
        for(i=0;i<x_cl;i++)
        {
            //t[i]=new float[y_cl];
            MPI_Recv(t,y_cl,MPI_FLOAT,root_id,i,MPI_COMM_WORLD,&st);
            for(j=0;j<y_cl;j++)
            {
                if(t[j]==out[i][j])
                {
    ...
    uint32 percentage_of_mach_found(float **out,float **level,uint16 x_cl,uint16 y_cl)
    {
        //float *t=new float[y_cl];
        uint32 sum=0;uint16 i,j;
        MPI_Status st;
        #pragma omp parallel for private(j,st)  reduction( + : sum )
        for(i=0;i<x_cl;i++)
        {
            //t[i]=new float[y_cl];
          //  MPI_Recv(t,y_cl,MPI_FLOAT,root_id,i,MPI_COMM_WORLD,&st);
...
//#pragma omp parallel for
    ...
    float dirivation_error(float **out,int root_id,uint16 x_cl,uint16 y_cl)
    {
        float *t=new float[y_cl];
        float dedout=0.0;
        uint16 multi=x_cl*y_cl;
        uint16 i,j;
        MPI_Status st;
//        cout<<x_cl<<"\t"<<y_cl;
#pragma omp parallel for private(j,t)  reduction( + : dedout )
        for( i=0;i<x_cl;i++)
        {
            MPI_Recv(t,y_cl,MPI_FLOAT,root_id,i,MPI_COMM_WORLD,&st);
            //dedout[i]=new float[y_cl]();
            for( j=0;j<y_cl;j++)
            dedout=dedout+2*(out[i][j]-t[j]);
        }
        MPI_Allreduce(&dedout,&dedout,1,MPI_DOUBLE,MPI_SUM, MPI_COMM_WORLD);
        //cout<<(dedout/multi);
        return (dedout/multi);

    }
    float dirivation_error(float **out,float **level,uint16 x_cl,uint16 y_cl)
    {
        float *t=new float[y_cl];
        float dedout=0.0;
        uint16 multi=x_cl*y_cl;
       ...
       // MPI_Status st;
//        cout<<x_cl<<"\t"<<y_cl;
#pragma omp parallel for private(j)  reduction( + : dedout )
        for( i=0;i<x_cl;i++)
        {
         //   MPI_Recv(t,y_cl,MPI_FLOAT,root_id,i,MPI_COMM_WORLD,&st);
            //dedout[i]=new float[y_cl]();
            for( j=0;j<y_cl;j++)
            dedout=dedout+2*(out[i][j]-level[i][j]);
        }
        //MPI_Allreduce(&dedout,&dedout,1,MPI_DOUBLE,MPI_SUM, MPI_COMM_WORLD);
        //cout<<(dedout/multi);
        return (dedout/multi);
