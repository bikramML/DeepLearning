...
#pragma omp parallel for
    ...
    uint32 percentage_of_mach_found(float **out,uint16 x_cl,uint16 y_cl,int tag_id)
    {
        float **t=new float*[x_cl];
        uint32 sum=0;
        MPI_Status st;uint16 i,j;
        #pragma omp parallel for private(j)  reduction( + : sum )
        for(i=0;i<x_cl;i++)
        {
            t[i]=new float[y_cl];
            MPI_Recv(t[i],y_cl,MPI_FLOAT,0,tag_id,MPI_COMM_WORLD,&st);
            for(j=0;j<y_cl;j++)
            {
                if(t[i][j]==out[i][j])
                {
...
//        MPI_Allreduce(&sum,&sum,1,MPI_UNSIGNED,MPI_SUM, MPI_COMM_WORLD);
    ...
    float dirivation_error(float **out,uint16 x_cl,uint16 y_cl,int tag_id)
    {
        float **t=new float*[x_cl];
        double dedout=0.0;MPI_Status st;
