...
private:
    ...
    void initilize_xmax_ymax(string file_name)
    {
        ...
        string grid_line="";
         int ii=0;
         bool check=false;
#pragma omp for private(grid_line)
         for(;std::getline(grid_fp,grid_line);)
         {
           if(ii==0)
           {
             ii++;
...
#pragma omp for private(line,cell)
...
#pragma omp for private(cell,rep,ci,p,fr,sc,pt,x1,y1,x,y)
...
#pragma  omp for private(c1) schedule(static)
...
#pragma  omp for private(c1,s) schedule(static)
        for (int r1 = 0; r1 < height; r1++)
        {
            if(r1<x_start)
                continue;
...
//        if(image_SWIR==nullptr)cout<<"aad";
    ...
    read_write(string file_name,uint16 image_size_x,uint16 image_size_y,uint16 x_start,uint16 y_start)
    {
        ...
        image_to_memory(folder+file_name,image_size_x,image_size_y,x_start,y_start);
      //  if(image_SWIR==nullptr)cout<<"aad";
